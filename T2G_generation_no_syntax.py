...
###############################################################################
WORKING_DIR = 'T2G_no_syntax'                       # NAME OF THE WORKING SPACE
...
TRAINED_MODEL_WEIGHTS = ['125'] #[str(i) for i in range(5,36, 5) ]             # SUBDIRS CONTAINING MODELS
...
def beam_search_generation( enc_word_ids, enc_input_att,
    ...
    encoder_embs = word_embedding_table(enc_word_ids)*32.
    encoder_outputs = encoder(None, inputs_embeds = encoder_embs,
                              attention_mask=enc_input_att)    
    
    [enc_word_ids, enc_input_att] = _extend_tensors([enc_word_ids, enc_input_att], batch_size, num_beams, input_seq_len)
...
for m in TRAINED_MODEL_WEIGHTS:
    ...
    for i in range(len(test_data_gen)):
        ...
        (enc_word_ids,enc_input_att), dec_input_ids  = test_data_gen[i]
        generated_tokens = beam_search_generation(enc_word_ids, enc_input_att)
