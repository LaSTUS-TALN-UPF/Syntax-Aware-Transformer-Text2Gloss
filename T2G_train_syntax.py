...
word_emb = word_embedding_table(tf.expand_dims([1], -1))
dep_emb = dep_embedding_table(tf.expand_dims([1], -1))

enc_inputs_embeds = tf.nn.l2_normalize(word_emb, axis = -1)+tf.nn.l2_normalize(dep_emb, axis = -1)
...
dec_inputs_embeds = tf.nn.l2_normalize(word_embedding_table(tf.expand_dims([1], -1)))
...
def forward_pass(enc_word_ids, enc_input_att, enc_dep_ids, dec_input_ids, training=False):
    
    word_emb = word_embedding_table(enc_word_ids)
    dep_emb = dep_embedding_table(enc_dep_ids)    
    enc_inputs_embeds = (word_emb+dep_emb)*32.0 # v1
...
for epoch in range(1,N_EPOCHS+1):
    ...
    t0 = time.time()
    loss = 0.0
    variables = word_embedding_table.trainable_variables+dep_embedding_table.trainable_variables +\
    ...
    if not epoch%5:
        ...
        for i in range(len(eval_generator)):
            ...
            logits = forward_pass(input_batch[0][0],
                                      ...
                                      input_batch[1],
                                      training = True)
            eval_loss += loss_function(input_batch[1], logits, input_batch[0][1])
        ...
        emb_weights = dep_embedding_table.get_weights()
        with open(handled_dirs['trained_models']+'/dep_embeddings_{}.weights'.format(epoch),'wb') as f:
            pickle.dump(emb_weights, f)        
        del emb_weights
